...
const errors={MessageNotFoundError:'Письмо больше не найдено в папке',LLMResponseFormatError:'LLM вернула ответ в неверном формате',AttachmentProcessingUnavailable:'Не удалось корректно обработать одно или несколько вложений',NodeExecutionBusy:'Узел уже выполняется; задача будет повторена',AbandonedNodeExecution:'Предыдущее выполнение узла было прервано; задача будет повторена',PermanentError:'Нужна ручная проверка',ExternalServiceError:'Внешний сервис временно недоступен',Exception:'Временная ошибка обработки'};
